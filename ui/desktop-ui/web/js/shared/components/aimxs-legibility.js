...
export function renderAimxsLegibilityBlock(model = {}) {
  ...
  const lifecycleTitle = String(model?.lifecycleTitle || "Decision Lifecycle").trim() || "Decision Lifecycle";
  const bindingTitle = String(model?.bindingTitle || "Decision Record").trim() || "Decision Record";
  const refsTitle = String(model?.refsTitle || "Evidence And Trace Refs").trim() || "Evidence And Trace Refs";
  return `
    <div class="aimxs-legibility-shell" data-aimxs-legibility="shared">
      ...
      <div class="aimxs-section">
        ...
        ${binding ? `<div class="aimxs-binding-grid">${binding}</div>` : '<div class="aimxs-empty">No decision-record fields are available yet.</div>'}
      </div>
      <div class="aimxs-section">
        <div class="aimxs-section-title">${escapeHTML(refsTitle)}</div>
        ${refs ? `<div class="aimxs-ref-group">${refs}</div>` : '<div class="aimxs-empty">No evidence or trace references are available yet.</div>'}
      </div>
