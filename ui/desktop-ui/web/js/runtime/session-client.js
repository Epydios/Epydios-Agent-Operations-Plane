...
const LEGACY_THREAD_INTENT_DEFAULTS = new Set([
  "Run an operator-guided governed agent conversation through the M16 session contract.".toLowerCase(),
  "Run an operator-guided agent conversation.".toLowerCase()
]);

function normalizeThreadIntent(value, fallback = "") {
  const text = normalizedString(value, fallback);
  if (!text) {
    return "";
  }
  if (LEGACY_THREAD_INTENT_DEFAULTS.has(text.toLowerCase())) {
    return "";
  }
  return text;
}

function displayPolicyProviderLabel(value) {
...
function defaultThreadIntent(scope = {}, draft = {}) {
  const projectId = normalizedString(scope?.projectId, "current project");
  const executionMode = normalizeExecutionMode(draft?.executionMode);
  if (executionMode === "managed_codex_worker") {
    return `Start a new governed Codex conversation for ${projectId}.`;
  }
  return `Start a new governed model conversation for ${projectId}.`;
}
...
function summarizeEventDetail(item) {
  ...
  if (type === "tool_proposal.decided") {
    ...
    const policyDecision = normalizedString(payload?.policyDecision).toUpperCase();
    const provider = displayPolicyProviderLabel(payload?.selectedPolicyProvider);
...
export function normalizeOperatorChatDraft(draft = {}, fallbackProfileId = "") {
  ...
  return {
    ...
    intent: normalizeThreadIntent(draft.intent),
...
export async function createOperatorChatThread(api, scope = {}, draft = {}) {
  ...
  const autogeneratedIntent = !normalizedDraft.intent;
  const normalizedIntent = normalizeThreadIntent(
    normalizedDraft.intent,
    defaultThreadIntent(scope, normalizedDraft)
  );
  ...
  const task = await api.createRuntimeTask({
    ...
    annotations: {
      ...
      threadIntent: {
        source: autogeneratedIntent ? "autogenerated" : "operator",
        autogenerated: autogeneratedIntent
      },
  ...
  return {
    ...
    intent: normalizeThreadIntent(task?.intent, normalizedIntent),
    tenantId,
    projectId,
...
export async function listOperatorChatThreads(api, scope = {}, options = {}) {
  ...
  const baseItems = (Array.isArray(response?.items) ? response.items : [])
    ...
    .map((item) => ({
      ...
      intent: normalizeThreadIntent(item?.intent),
...
export async function loadOperatorChatThread(api, scope = {}, taskOrId, options = {}) {
  ...
  return {
    ...
    intent: normalizeThreadIntent(task?.intent, normalizedString(latestTimeline?.task?.intent)),
    tenantId,
    projectId,
