...
func main() {
	...
	assert(strings.Contains(reportOutput, "AgentOps CLI governance report"), "expected CLI governance report header")
	assert(strings.Contains(reportOutput, "Applicable policy packs:"), "expected policy pack section")
	assert(strings.Contains(reportOutput, "Evidence preview: audit_bundle | evidence-cli-proof-1 | memory://evidence-cli-proof-1"), "expected evidence preview in report output")
	logger.log("proved audit and evidence handoff through cli-agentops report flow")
	...
	checklist := map[string]any{
		...
		"supported_path_cli_governed_thread": map[string]any{
			...
			"steps": []string{
				...
				"audit and evidence handoff rendered explicitly from cli-agentops report output",
			},
