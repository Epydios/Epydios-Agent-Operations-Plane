# EpydiosOps ChatOps Governed Thread Client

`chatops-agentops` is the bounded chatops client for the EpydiosOps governed-thread runtime contract. It does not create a separate orchestration layer. Instead, it uses the same governed task, session, worker, timeline, approval, tool-action, evidence, and event-stream APIs already used by the desktop shell, VS Code, CLI, and workflow clients.
...
- `conversations intake`
  ...
  - supports `text`, `json`, or normalized governed-thread `update` output
- `conversations status`
  ...
  - report output adds policy-pack and worker-capability coverage from the same governed-thread runtime contract
...
If more than one pending approval or proposal matches the resolved thread, rerun with the explicit `--checkpoint-id` or `--proposal-id` shown in `conversations status`. The governed-thread update output now includes those action hints directly.
